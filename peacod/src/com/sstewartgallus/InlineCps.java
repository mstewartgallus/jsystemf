...
public class InlineCps {
    ...
    private static <A> Value<U<A>> inlineThunkValue(Environment env, ThunkValue<A> thunk) {
        var jump = inline(env, thunk.action());
    ...
    private static <A> Instr inlineApplyKont(Environment env, ApplyKont<A> applyKont) {
        var x = inline(env, applyKont.action());
        var f = inlineStack(env, applyKont.next());

        if (x instanceof KontAction<A> kontAction) {
            var label = kontAction.label();
            // fixme.. check if it is a single use kont....
            System.err.println("fixme.. check if label is only used once or less....");
            return inlineStack(env.put(label, f), kontAction.body());
        }
    ...
    private static <A> Action<A> inline(Environment env, Action<A> action) {
    ...
    private static <A, B> Action<Fn<A, B>> inlineLambdaAction(Environment env, LambdaAction<A, B> lambdaAction) {
        ...
        var jump = inline(env, lambdaAction.body());
        return new LambdaAction<A, B>(binder, jump);
    }
