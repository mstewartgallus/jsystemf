...
public record CallThunk<Z, A, B>() implements ThunkTerm<V<Z, V<A, V<B, F<F<Z, F<A, B>>, F<F<Z, A>, F<Z, B>>>>>>> {
    @Override
    public <C> Term<C> stepThunk(Function<ValueTerm<V<Z, V<A, V<B, F<F<Z, F<A, B>>, F<F<Z, A>, F<Z, B>>>>>>>, Term<C>> k) {
        return k.apply(Term.v((Type<Z> z) -> Term.v((Type<A> a) -> Term.v((Type<B> b) -> {
            var fT = z.to(a.to(b));
            var xT = z.to(a);
